...
tic
parpool(16);
parfor i = 1 : 26%size(temp,2)
    i+8*num_of_subject
    original_signal = normalize_column(data(i+8*num_of_subject).time_series');
%     for j = 1 : (size(original_signal, 2)- window_size)
    for j = 1 : 350
        temp = abs(data(i+8*num_of_subject).fa_sc);
        if any(isnan(temp(:)))
            temp(isnan(temp)) = 0;
        end
        temp = Laplacian(temp);
...
% [L{i,j}, S{i,j}] = dynamic_graph_learning(original_signal(:,(j:window_size+j-1)), 0, 0.5, 0, 2);    
        [L9{i,j}, S9{i,j}] = dynamic_graph_learning(original_signal(:,(j:window_size+j-1)), temp, 0.5, 0.2, 100);
    end
