...
L7 = cell(num_of_subject,350);
S7 = cell(num_of_subject,350);
...
parfor i = 1 : num_of_subject%size(temp,2)
    i+6*num_of_subject
    original_signal = normalize_column(data(i+6*num_of_subject).time_series');
%     for j = 1 : (size(original_signal, 2)- window_size)
    for j = 1 : 350
        temp = abs(data(i+3*num_of_subject).fa_sc);
        if any(isnan(temp(:)))
            temp(isnan(temp)) = 0;
        end
        temp = Laplacian(temp);
...
% [L{i,j}, S{i,j}] = dynamic_graph_learning(original_signal(:,(j:window_size+j-1)), 0, 0.5, 0, 2);    
        [L7{i,j}, S7{i,j}] = dynamic_graph_learning(original_signal(:,(j:window_size+j-1)), temp, 0.5, 0.2, 100);
    end
